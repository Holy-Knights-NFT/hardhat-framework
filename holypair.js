    ...
    return keccak256(
        defaultAbiCoder.encode(
            ...
            [
                ...
                keccak256(ethers.utils.toUtf8Bytes("Pandora V1")),
                chainId,
                address,
            ]
        )
...
function getPandoraApprovalDigest(pandora, user, masterContractAddress, approved, nonce, chainId = 1) {
    ...
    const msg = defaultAbiCoder.encode(
        ...
        [
            ...
            keccak256(ethers.utils.toUtf8Bytes(approved ? "Give FULL access to funds in (and approved to) Pandora?" : "Revoke access to Pandora?")),
            user.address,
...
// Function on Pandora
const ACTION_PANDORA_DEPOSIT = 20;
const ACTION_PANDORA_WITHDRAW = 21;
const ACTION_PANDORA_TRANSFER = 22;
const ACTION_PANDORA_TRANSFER_MULTIPLE = 23;
const ACTION_PANDORA_SETAPPROVAL = 24;
...
class HolyPair {
    ...
    as(from) {
        let connectedPair = new HolyPair(this.contract.connect(from))
        connectedPair.pandora = this.pandora.connect(from)
    ...
    async run(commandsFunction) {
        ...
        for (let i = 0; i < commands.length; i++) {
            if (typeof commands[i] == "object" && commands[i].type == "HolyPairCmd") {
            ...
            } else if (typeof commands[i] == "object" && commands[i].type == "HolyPairDo") {
    ...
    tokenPermit(token, owner, owner_key, amount, nonce, deadline) {
        ...
        //return token.permit(addr(owner), addr(this.pandora), amount, deadline, v, r, s);
        let data = token.interface.encodeFunctionData("permit", [addr(owner), addr(this.pandora), amount, deadline, v, r, s])
    ...
    approveAsset(amount) {
        return this.asset.approve(this.pandora.address, amount)
    }

    approveCollateral(amount) {
        return this.collateral.approve(this.pandora.address, amount)
    }

    depositCollateral(amount) {
        return this.contract.cook(
            [ACTION_PANDORA_DEPOSIT, ACTION_ADD_COLLATERAL],
            [0, 0],
            [
                defaultAbiCoder.encode(
    ...
    withdrawCollateral(share) {
        return this.contract.cook(
            [ACTION_REMOVE_COLLATERAL, ACTION_PANDORA_WITHDRAW],
            [0, 0],
            [
    ...
    depositAssetWithApproval(amount, masterContract, privateKey, nonce) {
        ...
        return this.contract.cook(
            [ACTION_PANDORA_SETAPPROVAL, ACTION_PANDORA_DEPOSIT, ACTION_ADD_ASSET],
            [0, 0, 0],
            [
                defaultAbiCoder.encode(
    ...
    depositAsset(amount) {
        return this.contract.cook(
            [ACTION_PANDORA_DEPOSIT, ACTION_ADD_ASSET],
            [0, 0],
            [
                defaultAbiCoder.encode(["address", "address", "int256", "int256"], [this.asset.address, addr(this.contract.signer), amount, 0]),
                defaultAbiCoder.encode(["int256", "address", "bool"], [-2, addr(this.contract.signer), false]),
    ...
    withdrawAsset(fraction) {
        return this.contract.cook(
            [ACTION_REMOVE_ASSET, ACTION_PANDORA_WITHDRAW],
            [0, 0],
            [
    ...
    repay(part) {
        return this.contract.cook(
            [ACTION_GET_REPAY_SHARE, ACTION_PANDORA_DEPOSIT, ACTION_REPAY],
            [0, 0, 0],
            [
    ...
    short(swapper, amount, minReturnedShare) {
        ...
        return this.contract.cook(
            [ACTION_BORROW, ACTION_PANDORA_TRANSFER, ACTION_CALL, ACTION_ADD_COLLATERAL],
            [0, 0, 0, 0, 0],
            [
...
Object.defineProperty(HolyPair.prototype, "cmd", {
    get: function () {
