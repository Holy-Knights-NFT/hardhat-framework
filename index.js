...
const { HolyPair } = require("./holypair")
...
const PANDORA_MASTER_APPROVAL_TYPEHASH = keccak256(
...
function getPandoraDomainSeparator(address, chainId) {
    return keccak256(
        defaultAbiCoder.encode(
            ["bytes32", "bytes32", "uint256", "address"],
            [keccak256(toUtf8Bytes("EIP712Domain(string name,uint256 chainId,address verifyingContract)")), keccak256(toUtf8Bytes("Pandora V1")), chainId, address]
        )
    )
}

function getPandoraApprovalDigest(pandora, user, masterContractAddress, approved, nonce, chainId = 1) {
    const DOMAIN_SEPARATOR = getPandoraDomainSeparator(pandora.address, chainId)
    const msg = defaultAbiCoder.encode(
        ["bytes32", "bytes32", "address", "address", "bool", "uint256"],
        [
            PANDORA_MASTER_APPROVAL_TYPEHASH,
            keccak256(toUtf8Bytes(approved ? "Give FULL access to funds in (and approved to) Pandora?" : "Revoke access to Pandora?")),
            user.address,
...
function getSignedMasterContractApprovalData(pandora, user, privateKey, masterContractAddress, approved, nonce) {
    const digest = getPandoraApprovalDigest(pandora, user, masterContractAddress, approved, nonce, user.provider._network.chainId)
    const { v, r, s } = ecsign(Buffer.from(digest.slice(2), "hex"), Buffer.from(privateKey.replace("0x", ""), "hex"))
    return { v, r, s }
}

async function setMasterContractApproval(pandora, from, user, privateKey, masterContractAddress, approved, fallback) {
    if (!fallback) {
        const nonce = await pandora.nonces(user.address)

        const digest = getPandoraApprovalDigest(pandora, user, masterContractAddress, approved, nonce, user.provider._network.chainId)
        const { v, r, s } = ecsign(Buffer.from(digest.slice(2), "hex"), Buffer.from(privateKey.replace("0x", ""), "hex"))

        return await pandora.connect(user).setMasterContractApproval(from.address, masterContractAddress, approved, v, r, s)
    }
    return await pandora
        .connect(user)
...
async function setHolyPairContractApproval(pandora, user, privateKey, holyPair, approved) {
    const nonce = await pandora.nonces(user.address)

    const digest = getPandoraApprovalDigest(pandora, user, holyPair.address, approved, nonce, user.provider._network.chainId)
    const { v, r, s } = ecsign(Buffer.from(digest.slice(2), "hex"), Buffer.from(privateKey.replace("0x", ""), "hex"))

    return await holyPair.connect(user).setApproval(user.address, approved, v, r, s)
}

async function holyPairPermit(pandora, token, user, privateKey, holyPair, amount) {
    ...
    return await holyPair.connect(user).permitToken(token.address, user.address, pandora.address, amount, deadline, v, r, s)
}
    ...
    return deployments.createFixture(async ({ deployments, getNamedAccounts, ethers }, options) => {
        ...
        const cmd = {
            ...
            addPair: async function (var_name, tokenA, tokenB, amountA, amountB) {
                ...
                const HolySwapPairMock = await ethers.getContractFactory("HolySwapPairMock")
                const holySwapPair = await HolySwapPairMock.attach(pair)
                addContract(thisObject, var_name, holySwapPair)

                await tokenA.transfer(holySwapPair.address, getBigNumber(amountA, await tokenA.decimals()))
                await tokenB.transfer(holySwapPair.address, getBigNumber(amountB, await tokenB.decimals()))

                await holySwapPair.mint(thisObject.alice.address)
                return holySwapPair
            },
            addHolyPair: async function (var_name, pandora, masterContract, asset, collateral, oracle, oracleData) {
                const helper = await HolyPair.deploy(pandora, masterContract, masterContract.factory, asset, collateral, oracle, oracleData)
                addContract(thisObject, var_name, helper)
                return helper
            },
        }

        await deployFunction("factory", "HolySwapFactoryMock")
...
module.exports = {
    ...
    getApprovalDigest,
    getApprovalMsg,
    getPandoraDomainSeparator,
    getPandoraApprovalDigest,
    holyPairPermit,
    getSignedMasterContractApprovalData,
    setMasterContractApproval,
    setHolyPairContractApproval,
    sansBorrowFee,
    sansSafetyAmount,
    encodePrice,
    roundBN,
